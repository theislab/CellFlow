...
class ValidationData(BaseDataMixin):
    ...
    def __post_init__(self):
        self.cell_data = np.asarray(self.cell_data)
        self.split_covariates_mask = np.asarray(self.split_covariates_mask)
        self.split_idx_to_covariates = {k: tuple(np.asarray(v)) for k, v in self.split_idx_to_covariates.items()}
        self.perturbation_covariates_mask = np.asarray(self.perturbation_covariates_mask)
        self.perturbation_idx_to_covariates = {
            k: tuple(np.asarray(v)) for k, v in self.perturbation_idx_to_covariates.items()
        }
        self.perturbation_idx_to_id = {k: np.asarray(v) for k, v in self.perturbation_idx_to_id.items()}
